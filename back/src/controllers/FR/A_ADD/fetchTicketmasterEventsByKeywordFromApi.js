...
const fetchTicketmasterEventsByKeywordFromApi = async (req, res) => {
    ...
    console.log(`⚙️ [A1] [PARAMÈTRES] Phrase recherchée sur TM : "${keyword}" | (API Key : ${apiKey})`);
    console.log("\n");

    try {
        while (page < totalPages) { 
            const url = `https://app.ticketmaster.com/discovery/v2/events?apikey=${apiKey}&locale=*&countryCode=FR&keyword=${keyword}&size=${size}&page=${page}`;
